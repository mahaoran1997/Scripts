...
app = "tomcat"
directory = "/mnt/ssd/haoran/perf_dacapo/"+app+"/"
stats = [{}]
columns = []
times = 4;
numof_younggen_size = 7 + 9 + 1


with open(app+"_perf_stats.csv","w") as csvfile:
    writer = csv.writer(csvfile)
    for i in range(1,numof_younggen_size + 1):
        stats.append({})
        for j in range(1,times+1):
            if i <= 7:
                f = open(directory+app+str(16 * (2 ** i))+"m_"+str(j)+".out")
            elif i <= 7+9:
                f = open(directory+app+str((i - 7) * 4)+"g_"+str(j)+".out")
            else:
                f = open(directory+app+"eps_"+str(j)+".out")
            lines = f.readlines()
            for line_num in range(5,37):
                words = lines[line_num].split()
                # if(len(columns) < 28):
                #     columns.append(words[1])
                if words[1] not in stats[i]:
                    stats[i][words[1]] = 0;
                stats[i][words[1]] += locale.atoi(words[0])
            words = lines[38].split()
            if words[1] not in stats[i]:
                stats[i][words[1]] = 0.0;
            stats[i][words[1]] += locale.atof(words[0])
            f.close()
        if i == 1:
            for id in stats[i]:
                columns.append(id)
